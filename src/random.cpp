...
double RandomNumbers::uniform_double(double lower, double upper){
	std::uniform_real_distribution<> dist_uni(lower,upper);
	return dist_uni(rng);
}
...
double RandomNumbers::normal(double mean, double sd){
	std::normal_distribution<> dist_norm(mean,sd);
	return dist_norm(rng);
}
...
int RandomNumbers::poisson(double mean){
	std::poisson_distribution<> dist_pois(mean);
	return dist_pois(rng);
}
